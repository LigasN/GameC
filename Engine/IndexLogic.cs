...
namespace Game.Engine
{
	...
	public partial class Index
	{
		...
		public static List<Skill> AllAvailableSkills(Player player)
		{
			List<Skill> tmp = new List<Skill>();
			List<Skills.SkillFactories.SkillFactory> SkillsFactories = new List<Skills.SkillFactories.SkillFactory>{
				new Skills.SkillFactories.BasicSpellFactory(),
				new Skills.SkillFactories.BasicWeaponMoveFactory(),
				new Skills.SkillFactories.RefinedMovementsFactory() };

			SkillsFactories.ForEach(delegate (Skills.SkillFactories.SkillFactory factory)
			{
				tmp.AddRange(factory.AvailableSkillsList(player));
			});
		...
		public static List<Skill> MagicSkill(Player player)
		{
			...
			while(newSkills.Count < 3)
			{
				...
				if(generated != null)
					newSkills.Add(generated);
				counter++;
				if(counter > 10)
					break; // safety check if there are less than three skills left in the library
			}
			return newSkills;
		}
		...
		public static Item RandomClassItem(Player player)
		{
			...
			else if(player.ClassName == "Warrior")
