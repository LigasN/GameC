...
namespace Game.Engine
{
	...
	partial class GameSession
	{
		private void UpdateLocations()
		{
			...
			if(playerPosLeft > 0 && mapMatrix.Matrix[playerPosTop, playerPosLeft - 1] > 0)
				AvailableMoves[2] = true;
			if(playerPosLeft < mapMatrix.Width && mapMatrix.Matrix[playerPosTop, playerPosLeft + 1] > 0)
				AvailableMoves[3] = true;
			if(playerPosTop > 0 && mapMatrix.Matrix[playerPosTop - 1, playerPosLeft] > 0)
				AvailableMoves[0] = true;
			if(playerPosTop < mapMatrix.Height && mapMatrix.Matrix[playerPosTop + 1, playerPosLeft] > 0)
		...
		public void RemoveItemFromGrid(string it, int i)
		{
			// inform the display that the item has been removed
			Image img = new Image();
			img.Source = new BitmapImage(new Uri(("Assets/" + it + ".png"), UriKind.Relative));
			img.Name = it;
			parentPage.ItemGrid.Children.Remove(img);
		...
		public List<StatPackage> ModifyDefensive(List<StatPackage> packs)
		{
			...
			if(items.Count > 0)
			{
				...
				{
					for(int i = 0; i < packs.Count; i++)
						packs[i] = item.ModifyDefensive(packs[i], GetActiveItemNames());
				}
			}
			return packs;
		}
		...
		public int ListBoxInteractionChoice(List<string> choices)
		{
			...
			inter.Finish();
			if(choices.Count < inter.ChosenNumber)
				return 0;
				...
				for(int j = 0; j < 5; j++)
				{
					...
					if(img != null)
					{
						if(img.Name != "")
						{
							...
							if(tmp.HpMod > 0)
								txt += "Health(+" + tmp.HpMod + ") ";
							if(tmp.HpMod < 0)
								txt += "Health(" + tmp.HpMod + ") ";
							if(tmp.StrMod > 0)
								txt += "Strength(+" + tmp.StrMod + ") ";
							if(tmp.StrMod < 0)
								txt += "Strength(" + tmp.StrMod + ") ";
							if(tmp.ArMod > 0)
								txt += "Armor(+" + tmp.ArMod + ") ";
							if(tmp.ArMod < 0)
								txt += "Armor(" + tmp.ArMod + ") ";
							if(tmp.PrMod > 0)
								txt += "Precision(+" + tmp.PrMod + ") ";
							if(tmp.PrMod < 0)
								txt += "Precision(" + tmp.PrMod + ") ";
							if(tmp.MgcMod > 0)
								txt += "Power(+" + tmp.MgcMod + ") ";
							if(tmp.MgcMod < 0)
								txt += "Power(" + tmp.MgcMod + ") ";
							if(tmp.StaMod > 0)
								txt += "Health(+" + tmp.StaMod + ") ";
							if(tmp.StaMod < 0)
								txt += "Health(" + tmp.StaMod + ") ";
							if(tmp.PublicTip != null)
								txt += "Bonus: " + tmp.PublicTip;
							SendText(txt);
						}
					}
				}
				...
				try
				{
					...
					if(monster != null)
					{
						...
						if(newBattle.battleResult)
						{
							...
							mapMatrix.Monsters[mapMatrix.Width * PlayerPosTop + PlayerPosLeft] = monster; // remember this monster until the next time
