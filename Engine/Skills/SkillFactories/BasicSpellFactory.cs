...
namespace Game.Engine.Skills.SkillFactories
{
	...
	class BasicSpellFactory : SkillFactory
	{
		public List<Skill> AvailableSkillsList(Player player)
		{
			...
			if(known == null) // no BasicSpells known - we will return one of them
			{
				...
				if(s3.MinimumLevel <= player.Level)
					tmp.Add(s3);
				return tmp; // use Index.RNG for safe random numbers
			}
		...
		private Skill CheckContent(List<Skill> skills) // wrapper method for checking
		{
			...
			{
				if(skill is FireArrow || skill is WindGust || skill is LightFlash || skill is FireArrowDecorator || skill is WindGustDecorator || skill is LightFlashDecorator)
					return skill;
			}
			return null;
		}
